...
import easy2shop.function.HackyViewPager;
...
public class ViewPagerActivity extends Base {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			J = new JSONArray(bundle.getString("jArr"));
			int position = bundle.getInt("position");
			adapter = new SamplePagerAdapter(J);
	...
	private class SamplePagerAdapter extends PagerAdapter {
		JSONArray J;
		public SamplePagerAdapter(JSONArray J){
			this.J = J;
		}
		...
		public View instantiateItem(ViewGroup container, int position) {
	     ...
	     // this is going to load a 30mb download...
	        try {
				Ion.with(context)
				.load(J.getJSONObject(position).getString("imgurl")).noCache()
				//.progressDialog(dlg)
				.setLogging("DeepZoom", Log.VERBOSE)
				.withBitmap().placeholder(R.drawable.loading)
				.deepZoom()
